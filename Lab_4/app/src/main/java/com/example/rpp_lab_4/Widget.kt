...
class Widget: AppWidgetProvider() {
    ...
    override fun onUpdate(
        ...
        val sp = context?.getSharedPreferences(
            DateConfig.WIDGET_PREF, Context.MODE_PRIVATE
        )
        for(id in appWidgetIds!!){
            ...
            val intent = Intent(context, DateConfig::class.java)
            ...
            val timer = object : CountDownTimer(
                getDateDif(
                    sp?.getLong(DateConfig.WIDGET_TIME_UNIX + id, Long.MAX_VALUE)?: Long.MAX_VALUE),
                1000*60
            ) {
                ...
                override fun onTick(millisUntilFinished: Long) {
                    widgetView.setTextViewText(
                        ...
                        getTimerText(sp?.getLong(
                            DateConfig.WIDGET_TIME_UNIX + id,
                            0)?: 0
                        )
                    )
                    appWidgetManager?.updateAppWidget(id, widgetView)
    ...
    override fun onDeleted(context: Context?, appWidgetIds: IntArray?) {
        ...
        appWidgetIds?.forEach {
            timers[it]?.cancel()
            sp?.edit()?.apply {
                remove(DateConfig.WIDGET_TIME_UNIX + it)
                remove(DateConfig.WIDGET_TIME + it)
            }?.apply()
        }
    }
}